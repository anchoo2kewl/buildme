...
func serveSPA(r chi.Router, distPath string) {
	...
	r.Get("/*", func(w http.ResponseWriter, r *http.Request) {
		...
		// Dynamic route fallback: replace numeric segments with "_"
		// to match SSG placeholder pages (e.g. /projects/9 → /projects/_)
...
// resolveDynamicRoute finds SSG placeholder pages for dynamic routes.
// First tries replacing numeric segments with "_" (e.g. /projects/9 → /projects/_).
// Then tries replacing any segment where the literal directory doesn't exist
// but a "_" placeholder does (e.g. /groups/ai-agent-lens → /groups/_).
func resolveDynamicRoute(absPath, urlPath string) string {
	parts := strings.Split(strings.Trim(urlPath, "/"), "/")

	// Pass 1: replace numeric segments with "_"
	changed := false
	for i, p := range parts {
	...
	// Pass 2: for each segment where the literal dir doesn't exist, try "_"
	parts = strings.Split(strings.Trim(urlPath, "/"), "/")
	for i := range parts {
		literalDir := filepath.Join(absPath, filepath.Join(parts[:i+1]...))
		if _, err := os.Stat(literalDir); err == nil {
			continue // literal directory exists, skip
		}
		original := parts[i]
		parts[i] = "_"
		candidate := filepath.Join(absPath, filepath.Join(parts...), "index.html")
