...
export const StatusBadge = component$<StatusBadgeProps>(({ status }) => {
  const config: Record<BuildStatus, { bg: string; label: string }> = {
    ...
    running: { bg: "bg-running/20 text-running", label: "Running" },
    success: { bg: "bg-success/20 text-success", label: "Success" },
    failure: { bg: "bg-failure/20 text-failure", label: "Failed" },
    cancelled: { bg: "bg-warning/20 text-warning", label: "Cancelled" },
    error: { bg: "bg-failure/20 text-failure", label: "Error" },
