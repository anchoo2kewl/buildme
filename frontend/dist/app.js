...
(function() {
  ...
  function renderDashboardTable(env) {
    ...
    for (const entry of data) {
      ...
      // Find the latest build for this env's branch
      ...
      const dur = formatDuration(build.duration_ms);
  ...
  // --- Project detail with build history + jobs ---
  ...
  function renderBuildHistory(branchFilter) {
    ...
    content.innerHTML = filtered.map(b => {
          ...
          <div class="job-list">
            ${jobs.map(j => `
              <div class="job-row job-${statusClass(j.status)}">
                ...
                <span class="job-dur">${formatDuration(j.duration_ms)}</span>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      return `
        <div class="build-hist" data-build-id="${b.id}">
          <div class="build-hist-header" onclick="this.parentElement.classList.toggle('expanded'); var body=this.nextElementSibling; if(body) body.style.display=body.style.display==='none'?'block':'none'">
          ...
          ${jobsHTML ? `<div style="display:none">${jobsHTML}</div>` : ''}
        </div>
      `;
    }).join('');
  }
    ...
    try {
      ...
      document.getElementById('settings-content').innerHTML = `
        ...
        <div class="card mb-4">
          ...
          <div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th></tr></thead><tbody>
            ${(members || []).map(m => `<tr><td>${esc(m.user?.display_name || m.user?.email || 'User #' + m.user_id)}</td><td><span class="badge badge-running">${esc(m.role)}</span></td></tr>`).join('')}
          </tbody></table></div>
        </div>
        <div class="card">
