...
import { z } from "zod";
import { BuildMeClient } from "./api.js";
...
function saveAgentName(apiKeyHash, agentName) {
...
function getPersistedAgentName(apiKeyHash) {
 ...
 */
function createServer(client, cachedUser) {
    const server = new McpServer({
        name: "buildme",
        version: "1.0.0",
    });
    // --- get_me ---
    server.tool("get_me", "Get current authenticated user info", { verbose: z.boolean().optional().describe("Return full details (default: false)") }, async ({ verbose }) => {
        const user = cachedUser ?? (await client.getMe());
        return { content: [{ type: "text", text: formatResponse(user, verbose) }] };
    });
    // --- get_dashboard ---
    server.tool("get_dashboard", "All projects + latest builds per env (staging/uat/prod)", {
    ...
    // --- get_drift ---
    server.tool("get_drift", "Drift: deployed vs built versions + health per env", {
        verbose: z.boolean().optional().describe("Return full details (default: false)"),
    }, async ({ verbose }) => {
    ...
    // --- sync_all ---
    server.tool("sync_all", "Trigger sync for all projects, returns results", {
        verbose: z.boolean().optional().describe("Return full details (default: false)"),
    }, async ({ verbose }) => {
    ...
    // --- get_project ---
    server.tool("get_project", "Project details by ID", {
    ...
    // --- list_builds ---
    server.tool("list_builds", "Builds for a project", {
        project_id: z.string().describe("Project ID"),
    ...
    // --- get_build ---
    server.tool("get_build", "Build detail with jobs", {
        project_id: z.string().describe("Project ID"),
        ...
        return { content: [{ type: "text", text: formatResponse(build, verbose) }] };
    });
    // --- sync_project ---
    server.tool("sync_project", "Trigger sync for one project", {
        project_id: z.string().describe("Project ID"),
        verbose: z.boolean().optional().describe("Return full details (default: false)"),
    }, async ({ project_id, verbose }) => {
    ...
    // --- health_check ---
    server.tool("health_check", "Server health status", {
        verbose: z.boolean().optional().describe("Return full details (default: false)"),
    }, async ({ verbose }) => {
    ...
    // --- get_build_stats ---
    server.tool("get_build_stats", "Aggregate: success rate, avg duration, recent failures", {
        ...
        if (project_id) {
            ...
            return { content: [{ type: "text", text: formatResponse(stats, verbose) }] };
        }
        else {
...
app.post("/mcp", async (req, res) => {
    ...
    const headerAgent = req.headers["x-agent-name"];
    if (headerAgent) {
    ...
    if (!client.agentName) {
        ...
        if (entry?.agentName)
            client.agentName = entry.agentName;
    }
    const keyHash = hashKey(apiKey);
    if (!client.agentName) {
        const persisted = getPersistedAgentName(keyHash);
        if (persisted)
            client.agentName = persisted;
    }
    if (client.agentName) {
        const entry = apiKeyCache.get(apiKey);
        if (entry)
            entry.agentName = client.agentName;
